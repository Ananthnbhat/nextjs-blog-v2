...
- `const`- the variable pointing to the data is locked.
- `let` - variable can be pointed to different data in memory (re-assignment)
...
Primitive values are immutable, so when you set state, the state has a new value with new reference, so there is a re-render. Whereas non-primitive values are mutable, so when you set state,
the value should not be mutated, instead set a new value to the state. This way a new reference gets created for that state value and re-render occurs.
...
A component is a re-usable UI element. For example, we may build a button component that can be used in multiple scenarios throughout an application. If a component is used in multiple places or scenarios, each time a new
long running object gets created storing contextual information about that particular component. So each component render is called component instance.
